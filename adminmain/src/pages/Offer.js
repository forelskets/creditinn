...
import { Edit, FormModal } from '../components/_dashboard/offer';
...
import { AllBankOffer, ChangeOfferStatus, saveBankOffer } from '../_services/Admin.services';

import { ApplicationsStateChange, DeleteBankOffer } from '../_services/Admin.services';
import { useDispatch } from 'react-redux';
import { NotificationMsg, NotificationTable } from '../components/_dashboard/offer/Notification';
// ----------------------------------------------------------------------
...
export default function Offer() {
      ...
      res?.data?.services?.map((ele, ind) => {
        ...
        return ele?.BankService?.map((elm, indx) => {
          data1.push({
            ...
            RTEditor: ele?.RTEditor,
            type: ele?.Type,
            id: ele?._id,
            Bank: ele?.BankName?.BankName,
            BankId: ele?.BankName?._id,
            Service: elm?.ServiceName,
            ServiceId: elm?._id,
            Note: ele?.Note,
            Category: ele?.Category,
            Picture: BannerImg?.filePath,
            View: ele?.Status
          });
        });
      });
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row) => (
        <>
          <Edit data={row} callApi={callEffect} />
        </>
      )
    },
    {
      ...
      render: (row) => (
        <>
          <RichTextEditor row={row} callApi={callEffect} />
        </>
      )
    }
  ];
