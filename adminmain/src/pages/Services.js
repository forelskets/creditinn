...
import { service, AllService , Updateservice ,Deleteservice , ChangeServiceStatus, AllCategory} from '../_services/Admin.services/index';
...
export default function Services() {
  ...
  const columns = [
    ...
    {title:"View" , field: "View" , render: (row) => <>{(row.View === true) ?<FormControlLabel control={<Switch checked={row.View} onChange={(e)=> StatusChangeHandler(e ,row)} />} label="Can-View" />: <FormControlLabel control={<Switch checked={row.View} onChange={(e)=> StatusChangeHandler(e, row)} />} label="Can't-View" />}</> , editable: 'never' , filtering : false},
    {title: "Category" , field: "CategorySelect" , lookup : categoryArray , validate: rowData => rowData.CategorySelect=== undefined ||rowData.CategorySelect==="" ? "required": true
  ...
  const callEffect = async () => {
    ...
    if (res?.status === 1 && Array.isArray(res?.data?.services)) {
      ...
      res.data.services.map((ele)=>{
            ...
            data1.push({"ServiceName": ele.ServiceName , "Note": ele.Note , "View": ele.Status , "_id": ele._id , "CategorySelect":ele.Category})
