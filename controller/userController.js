...
const Phone = require("../routers/util/Phone");
...
exports.ShareRefralDataStor = async (req, res, next) => {
  ...
  try {
    const result = await ShareRefral({
      userId: id,
      Name: name,
      Mobile: mobile,
      Product: product,
...
exports.getAllRefrals = async (req, res, next) => {
  ...
  try {
    const result = await ShareRefral.find().populate({
      path: "userId",
      select: "-_id Name",
    });
    if (result) {
...
module.exports.retrieveUser = async (req, res, next) => {
  ...
  try {
    const user = await User.findOne({ Email }, "UserId Name Email Password");
    updatedFields = user;
