...
export default function Token() {
  ...
  const TABLE_HEAD = [
    ...
    { field: 'createdAt', title: 'Date& Time' },
    {
      ...
      render: (row) => (
        <>
          <Status id={row?.id} value={row?.Status} ApiUpdate={getUsers} />
        </>
      )
    },
    {
      ...
      title: 'Modal',
      render: (row) => (
        <>{row?.Status === 'Approved' ? <FormModal data={row} callApi={getUsers} /> : ''}</>
      )
    }
  ];
  ...
  return (
    <Page title="Insurances Application">
      <div style={{ display: 'flex' }}>
        ...
        <Button variant="contained" onClick={() => setUserList(realData)}>
          Refresh
        </Button>
      </div>
